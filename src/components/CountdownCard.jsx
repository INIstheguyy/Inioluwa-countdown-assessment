...
import { getProgressPercent, formatRelativePast, formatTargetDate } from '../utils/timeUtils';
...
export default function CountdownCard({ countdown, positionClass, onEdit, onDelete, compact = false }) {
  const [confirmDelete, setConfirmDelete] = useState(false);
  const { days, hours, minutes, seconds, isExpired, urgency } = useCountdownTimer(countdown.targetDate);
  const config   = URGENCY_CONFIG[urgency];
  const progress = getProgressPercent(countdown.createdAt, countdown.targetDate);
    ...
    >
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom: compact ? '12px' : '20px' }}>
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:'6px', marginBottom:'4px' }}>
            {countdown.emoji && (
              <span style={{ fontSize: compact ? '16px' : '20px', lineHeight:1 }}>{countdown.emoji}</span>
            )}
            <h2 style={{
              fontFamily:    'var(--font-display)',
        ...
        <div style={{ display:'flex', gap:'5px', marginLeft:'10px', flexShrink:0 }}>
          {!isExpired && (
            <button
              ...
              style={{
                ...
                color:'var(--ink-3)', cursor:'pointer', fontSize:'13px',
          ...
          {!confirmDelete ? (
            <button
              ...
              onMouseEnter={e => { e.currentTarget.style.color='var(--coral)'; e.currentTarget.style.borderColor='var(--coral)'; e.currentTarget.style.background='var(--coral-lt)'; }}
              onMouseLeave={e => { e.currentTarget.style.color='var(--ink-3)'; e.currentTarget.style.borderColor='var(--border)'; e.currentTarget.style.background='var(--bg)'; }}
            >
              ✕
            </button>
          ) : (
            <div style={{ display:'flex', gap:'4px', alignItems:'center' }}>
              <span style={{ fontSize:'10px', color:'var(--coral)', fontFamily:'var(--font-mono)', whiteSpace:'nowrap' }}>Sure?</span>
              <button
                onClick={() => onDelete(countdown.id)}
                style={{
                  padding:'3px 8px', borderRadius:'6px', border:'none',
                  background:'var(--coral)', color:'white', fontSize:'10px',
              ...
              <button
                ...
                style={{
                  ...
                  color:'var(--ink-2)', fontSize:'10px', fontWeight:500,
      ...
      {isExpired ? (
        <div style={{
          ...
          <div style={{
            fontFamily:'var(--font-display)', fontSize: compact ? '16px' : '22px',
            fontWeight:700, color:'var(--ink-3)', letterSpacing:'-0.02em',
          }}>
            {countdown.name}
          </div>
          <div style={{
            fontFamily:'var(--font-mono)', fontSize:'11px',
            color:'var(--expired)', fontWeight:500,
          }}>
